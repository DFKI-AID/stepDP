...
import de.dfki.step.core.HypothesisProcessor;
...
import de.dfki.step.output.impp.Allocation;
    ...
    public Optional<DialogFunction> process(Inputs inputs) {
        ...
        return Optional.of(new DeviceControlDC.AbsDialogFunction(DeviceControlDC.this, currentInput) {
            ...
            public void run() {
                ...
                Allocation id = null;
//                if(matchedHypo.getIntent().equals("turnOn")) {
//                    id = imp.allocate(OutputNode.build(new SpeechOutput("ok, I will turn on the  " + matchedHypo.getSlots().get("dev").getFinalSlotEntity().get().get(Ontology.id).get())).build());
//                } else if(matchedHypo.getIntent().equals("turnOff")) {
